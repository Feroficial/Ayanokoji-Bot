...
const preguntas = [
    ...
    { pregunta: "¿Quién wrote 'La Divina Comedia'?", respuesta: "dante", opciones: ["Dante Alighieri", "Petrarca", "Boccaccio", "Ariosto"] },
...
> ₊· ⫏⫏ ㅤ Responde con: #r <respuesta>
        `.trim(), vsBot ? {} : { mentions: [m.sender, opponent] })
...
ㅤ    ꒰  ㅤ ✿ ㅤ *αℓуα - вσт* ㅤ ⫏⫏ ꒱
            `.trim(), ganador === 'bot' ? {} : { mentions: [ganador] })
...
async function enviarPregunta(m, game) {
    const turnoNombre = game.turn === 'bot' ? '🤖 αℓуα - вσт' : `@${game.turn.split('@')[0]}`
...
> ₊· ⫏⫏ ㅤ Se le otorga un punto al oponente
