...
const handler = async (m, { conn, args}) => {
  try {
    await conn.reply(m.chat, '⏳ *_Actualizando la bot... Por favor espera._*', m);

    const output = execSync('git pull' + (args.length? ' ' + args.join(' '): '')).toString();
    const isUpdated = output.includes('Already up to date');

    const updateMsg = isUpdated
? '✅ *Alya Bot ya está actualizado.*'
...
!file.startsWith('tmp/')
        ...
        if (conflictedFiles.length> 0) {
          conflictMsg = `⚠️ *Conflictos detectados en los siguientes archivos:*\n\n` +
            ...
            `\n\n🔧 *Solución recomendada:* reinstala la bot o resuelve los conflictos manualmente.`;
}
}
} catch (statusError) {
